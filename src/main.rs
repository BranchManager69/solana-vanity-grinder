...
fn main() {
    ...
    if args.len() < 2 {
        ...
        println!("{}", "-----------------------------".green());
        println!("Commands:");
        println!("  {}            - Run a performance benchmark on your system", "benchmark".cyan());
        println!("  {}    - Estimate time to find an address with pattern of specified length", "estimate <length>".cyan());
        println!("  {}  - Generate an address with specified prefix", "generate <pattern>".cyan());
        println!("  {}      - Generate with pattern at the end of address", "--suffix <pattern>".cyan());
        println!("  {} - Case-insensitive search (faster)", "--no-case-sensitive".cyan());
        println!("");
        println!("Examples:");
        println!("  {}       - Find address starting with 'DEGEN'", "cargo run -- generate DEGEN".cyan());
        println!("  {}  - Find address ending with 'MOON'", "cargo run -- generate --suffix MOON".cyan());
        println!("  {} - Find address with 'BTC' (case-insensitive)", "cargo run -- generate BTC --no-case-sensitive".cyan());
        process::exit(0);
    }
    ...
    match args[1].as_str() {
        "benchmark" => {
            ...
            match cuda_helpers::CudaDevice::new() {
                Ok(device) => {
                    ...
                    match cuda_helpers::find_optimal_batch_size(&device) {
        ...
        _ => {
            // Try to interpret as a legacy command for backward compatibility
...
/// Formats a number with commas for easier reading and no decimals for large numbers
